...
void tmr_setup_period(int timer, int ms){
    ...
    long int clk_steps = (Fcy/64)*(ms/1000.0);
    switch(timer){
        case 1:{
            TMR1 = 0; //reset timer
            PR1 = clk_steps; // set value to count up to
            T1CONbits.TCKPS = 2; //set prescaler
        ...
        case 3:{
            ...
            T3CONbits.TCKPS = 2; //set prescaler
...
void tmr_wait_period(int timer){
    //function that check if a timer is expired otherwise wait for it to expire.
    switch(timer){
        case 1:{
            while(1){
                if(IFS0bits.T1IF){
