...
int main(void) {
    ...
    while(1){
        button_info = PORTEbits.RE8 ; //read the button 
        
        if (button_info == 0 && button_state!=0){
            //if the button is press increment the pulse             
            if(pulse<3){
                pulse++;
            ...
            cycle = 1; //start the 1000ms period from now on
        
        }
        
        button_state=button_info; //update the state of the button
        ...
        if(cycle == 11){
            //reset the cycle if it execedes 10
            cycle = 1; 
        }
        
        tmr_wait_period(TIMER1); //wait for the 100ms period to end
